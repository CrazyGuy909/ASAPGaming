...
SWEP.PrintName = "Poison Drill" -- Weapon name (Shown on HUD)
...
SWEP.Primary.RPM_Semi = 370 -- RPM for semi-automatic or burst fire.  This is in Rounds Per Minute / RPM
...
function SWEP:Initialize()
    ...
    hook.Add("EntityTakeDamage", self, function(_, ent, dmg)
        if ((ent:IsPlayer() or ent:IsNPC()) and dmg:GetInflictor() == self and dmg:GetDamageType() != DMG_POISON) then
            ent:CreatePoison(5, self:GetOwner(), self, .075)
        end
    end)
end
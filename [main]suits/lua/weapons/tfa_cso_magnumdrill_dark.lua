...
SWEP.PrintName = "Shadow Drill" -- Weapon name (Shown on HUD)
...
function SWEP:Initialize()
    ...
    hook.Add("EntityTakeDamage", self, function(_, ent, dmg)
        if (ent:IsPlayer() and dmg:GetInflictor() == self) then
            if ((ent._darkCooldown or 0) > CurTime()) then return end
            ent:CreateShadowBlind(5)
            ent._darkCooldown = CurTime() + 10
        end
    end)
end
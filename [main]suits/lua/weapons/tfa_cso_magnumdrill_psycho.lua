...
SWEP.PrintName = "Ultimate Drill" -- Weapon name (Shown on HUD)
...
SWEP.Primary.IronAccuracy = .0225 -- Ironsight accuracy, should be the same for shotguns
...
--
SWEP.Skin = 2
SWEP.ViewModel = "models/weapons/tfa_cso/c_magnum_drill_expert_asap.mdl" --Viewmodel path
...
--[[WORLDMODEL]]
--
SWEP.WorldModel = "models/weapons/tfa_cso/w_magnum_drill_expert_asap.mdl" -- Worldmodel path
...
SWEP.Secondary.BashDelay = 0.4355
SWEP.Secondary.BashDamage = 200
SWEP.Secondary.BashLength = 140
SWEP.Secondary.BashSound = Sound("MagnumDrill.DrillExpert")
...
function SWEP:Holster(...)
    self:StopSound("MagnumDrill.IdleExpert")
...
function SWEP:Initialize()
    ...
    hook.Add("EntityTakeDamage", self, function(_, ent, dmg)
        if ((ent:IsPlayer() or ent:IsNPC()) and dmg:GetInflictor() == self and (ent.lastKnockup or 0) < CurTime()) then
            ent.lastKnockup = CurTime() + 5
            local dir = (ent:GetPos() - self:GetOwner():GetPos()):GetNormalized()
            if ent:IsOnGround() then
                ent:SetPos(ent:GetPos() + Vector(0, 0, 2))
            end
            ent:SetLocalVelocity(Vector(0, 0, 0))
            ent:Wait(0, function()
                ent:SetLocalVelocity(dir * 125 + Vector(0, 0, 175))
            end)
        end
    end)

    hook.Add("EntityFireBullets", self, function(_, ent, data)
        if (ent:IsPlayer() and ent:GetActiveWeapon() == self) then
            data.Force = .05
        end
    end)
end
...
function EFFECT:Init(data)
    ...
    local noloop = false
    hook.Add("PrePlayerDraw", self, function(_, ply)
        if (noloop) then return end
        if (ply == ent) then
            noloop = true
            self:RenderParent(ply)
            noloop = false
...
function EFFECT:RenderParent(ply)

    local bClipping = self:StartClip(self, 1)
    ply:DrawModel()
    render.PopCustomClipPlane()
    render.EnableClipping(bClipping)
    self:RenderOverlay(ply)
end

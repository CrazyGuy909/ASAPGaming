...
function ENT:Initialize()
    ...
    self:StartLoopingSound("npc/ichthyosaur/water_growl5.wav")
    ...
    if SERVER then
        ...
        for k, v in pairs(ents.FindInSphere(self:GetPos(), self.MaxSize * size)) do
            ...
            if (v:IsPlayer() or v:IsNPC()) then
                ...
                if (v:IsPlayer() and self:GetOwner():GetGang() ~= "" and self:GetOwner():GetGang() == v:GetGang()) then continue end
                local heal = v:Health() * .075
                v:TakeDamage(heal)
                self:GetOwner():SetHealth(math.Clamp(self:GetOwner():Health() + heal, 0, 2000))
                self:GetOwner():TakeDamage(0)

                if (math.random(1, 3) == 1) then
                    v:EmitSound("npc/headcrab_poison/ph_poisonbite" .. math.random(1, 3) .. ".wav", 75, math.random(85, 115), 1)
                end
            end
        end
...
local sphere = Material("asap/red_bubble")
...
function ENT:DrawTranslucent()
    ...
    if not self.Clouds then
        ...
        for k = 1, 4 do
            ...
            self.Clouds[k]:SetMaterial("asap/particles/smokesprites_000" .. k)
            self.Clouds[k]:SetColor(Color(100, 25, 25, 255))

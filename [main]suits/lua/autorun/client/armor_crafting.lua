...
function PANEL:Init()
    ...
    self.Items.Paint = function(s, w, h)
        draw.RoundedBox(8, 0, 0, w, h, Color(16, 16, 16))
        if (self.Price) then
            draw.SimpleText("Upgrade Price", "Gangs.Small", w / 2, h - 42, color_white, 1, TEXT_ALIGN_BOTTOM)
            draw.SimpleText(DarkRP.formatMoney(self.Price), "Gangs.Medium", w / 2, h - 8, color_white, 1, TEXT_ALIGN_BOTTOM)
        end
    end
    ...
    self.ModelController.PaintOver = function(s, w, h)
        draw.SimpleText(self.Title, "Gangs.Small", w / 2, 16, self.Color or color_white, 1)
    end
...
function PANEL:PopulateRecipes()
    ...
    for k, v in pairs(Armor.Crafting.Suits) do
        local item = BU3.Items.Items[k]
        if not item then continue end
        if (filter and filter ~= "" and not string.find(string.lower(item.name), filter, 1, true)) then continue end
        self:CreateRecipe(i, k, v, item, 1)
        i = i + 1
    end

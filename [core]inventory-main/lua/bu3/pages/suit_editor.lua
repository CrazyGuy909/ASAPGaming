...
function PAGE:Load(contentFrame, itemData)
	...
	PAGE.ItemData.type = "suit"
	PAGE.ItemData.isFeatured = false
	PAGE.ItemData.max = 0
	PAGE.ItemData.isNew = false
	...
	textPanel.Paint = function(s , w , h)
		draw.SimpleText("SUIT EDITOR", BU3.UI.Fonts["large_bold"], w/2, h/2, Color(255,255,255,20),1 ,1)
	end
	...
	local weaponClassEntry = BU3.UI.Elements.CreateTextEntry("Suit Class Name", self.pages, false, false)
	...
	iconColorCodeSelect:AddChoice("Diamond")
	iconColorCodeSelect:AddChoice("Fire")
	iconColorCodeSelect:AddChoice("Rainbow")
	iconColorCodeSelect:AddChoice("Glitched")
	iconColorCodeSelect:AddChoice("Green")
	...
	isNewCheckbox.Think = function(s)
		PAGE.ItemData.isNew = s.state
	end
	isNewCheckbox.state = PAGE.ItemData.isNew
	...
	isFeaturedCheckbox.Think = function(s)
		PAGE.ItemData.isFeatured = s.state
	end
	isFeaturedCheckbox.state = PAGE.ItemData.isFeatured

	local maxText = vgui.Create("DPanel", self.pages)
	maxText:SetSize(200, 40)
	maxText:SetPos(baseOffset + 235 + 370, 260 + 64)
	maxText.Paint = function(s , w , h)
		draw.SimpleText("Max (0 infinite)", BU3.UI.Fonts["small_bold"], 0, 0, Color(210,210,210,255),0 ,0)
	end	

	local maxEntryBox = BU3.UI.Elements.CreateTextEntry("'100'", self.pages, false, false)
	maxEntryBox:SetSize(50 + 15, 25)
	maxEntryBox:SetPos(baseOffset + 525 + 300,260 + 64)
	maxEntryBox:SetText(PAGE.ItemData.max)
	maxEntryBox.OnValueChange = function(s , val)
		PAGE.ItemData.max = tonumber(val)
	end
	...
	local createCaseButton = BU3.UI.Elements.CreateStandardButton("Create/Update Suit", self.pages, function()
	...
	createCaseButton:SetPos(baseOffset + (contentFrame:GetWide()/2) - 178, 370 + 148 + 32)
end
...
--Register the page
BU3.UI.RegisterPage("suiteditor", PAGE)
...
function PAGE:Load(contentFrame, itemData)
	...
	PAGE.ItemData.name = ""
	PAGE.ItemData.desc = ""
	PAGE.ItemData.iconID = "attachment"
	...
	textPanel.Paint = function(s , w , h)
		draw.SimpleText("ATTACHMENT EDITOR", BU3.UI.Fonts["large_bold"], w/2, h/2, Color(255,255,255,20),1 ,1)
	end
	...
	if not PAGE.ItemData.iconIsModel then
		previewImage = BU3.UI.Elements.IconView(PAGE.ItemData.iconID , color_white, self.pages, true)
	...
	if PAGE.ItemData.iconID ~= "attachment" then
	...
	local createCaseButton = BU3.UI.Elements.CreateStandardButton("Create/Update Att", self.pages, function()
	...
	createCaseButton:SetSize(220, 40)
	createCaseButton:SetPos(baseOffset + (contentFrame:GetWide() /2) - 64 + 42, 400)
end
...
--Register the page
BU3.UI.RegisterPage("attachmenteditor", PAGE)
...
function LOUNGE_CHAT:OnPlayerChat(ply, text, bteam, bdead, preftext, prefcolor, color)
	...
	if (IsValid(ply)) then
		...
		local cr = nil  -- Initialize cr at the beginning for each player
		if ply.donatorVisual == 0 then
			cr = "User | "
		elseif ply.donatorVisual == 1 then
			cr = "<flash=ddff00,2> VIP </flash> | "
		elseif ply.donatorVisual == 2 then
			cr = "<flash=ff9100,2> Ultra VIP </flash> | "
		elseif ply.donatorVisual == 3 then
			cr = "<flash=ff0000,2> Ambassador </flash> | "
		elseif ply.donatorVisual == 4 then
			cr = "<flash=ffe100,2> Executive </flash> | "
		elseif ply.donatorVisual == 5 then
			cr = "<flash=d400ff,2> Galaxy </flash> | "
		end

		if cr then 
			Add(cr)  -- Add your logic to handle the cr value for this player
		end
	end

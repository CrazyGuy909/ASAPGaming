...
timer.Create(tag, 5, 1, function()
    if hadEternal then
        ASAPDriver:MySQLUpdate("nebula_eternals", {progress = self._eternals[id].progress, level = self._eternals[id].level}, "steamid = " .. self:SteamID64() .. " and id = '" .. id .. "'")
    else
        ASAPDriver:MySQLInsert("nebula_eternals", {steamid = self:SteamID64(), id = "'" .. id .. "'", progress = self._eternals[id].progress, level = self._eternals[id].level})
    end
end)
...
hook.Add("DatabaseInitialized", "NebulaEternals.AddDB", function()
    ASAPDriver:MySQLCreateTable("eternals", {
        steamid = "VARCHAR(22)",
        id = "VARCHAR(32)",
    ...
    ASAPDriver:MySQLHook("eternals", function(ply, data)
        ply._eternalsData = {}

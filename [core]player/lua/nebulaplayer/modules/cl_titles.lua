...
local function OpenCosmeticTitlePanel()
    -- Check if the panel is already open to prevent duplicates
    if IsValid(LocalPlayer().CosmeticTitlePanel) then
        LocalPlayer().CosmeticTitlePanel:Remove()
    end

    -- Create the cosmetic title panel and set it as a member of the player entity
    LocalPlayer().CosmeticTitlePanel = vgui.Create("nebula.cosmeticTitle")
end

hook.Add("PlayerSay", "OpenCosmeticTitlePanels", function(ply, text)
    if (text == "!titles") then
		print("!titles command received")

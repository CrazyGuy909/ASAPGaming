...
function NebulaPremium:CreateLog(receiver, sender, amount, source)
    ASAPDriver:MySQLInsert("premium_logs", {
        receiver = receiver,
        sender = sender,
        source = source,
...
function meta:setCosmeticTitle(text, animation, style)
    ...
    ASAPDriver:MySQLUpdate("premium", {
...
hook.Add("DatabaseInitialized", "NebulaRP.Store", function()
    ASAPDriver:MySQLCreateTable("premium", {
    ...
    ASAPDriver:MySQLCreateTable("premium_logs", {
    ...
    ASAPDriver:MySQLHook("premium", function(pl, data)
        local time = os.time()

        if not data then
            ASAPDriver:MySQLInsert("premium", {
...
concommand.Add("neb_addrank", function(ply, cmd, args)
    ...
    if IsValid(player.GetBySteamID64(target)) then
        ...
        ASAPDriver:MySQLQuery("SELECT titles FROM premium WHERE steamid = " .. target, function(data)
...
concommand.Add("neb_addcredits", function(ply, cmd, args)
    ...
    if IsValid(player.GetBySteamID64(target)) then
        ...
        ASAPDriver:MySQLQuery("SELECT credits FROM premium WHERE steamid = " .. target, function(data)
            if data and data[1] then
                ASAPDriver:MySQLUpdate("premium", {
...
--     else
--         ASAPDriver:MySQLQuery("SELECT bag FROM premium WHERE steamid = " .. target, function(data)
--             if data and data[1] then
